...
# Create a directory for the output files of this step:
dir.create(path = "./2_LongestIsoforms/")

# For each species i in the list of species, species:
for (i in species) {
  print(i)
  filteredTranscriptsOutput <- paste("./2_LongestIsoforms/",i,"_filteredTranscripts.fasta", sep = "")
  if (file.exists(filteredTranscriptsOutput)) {
    message <- paste("The filtered transcript file", filteredTranscriptsOutput, "already exists.", sep = " ")
    print(message)
  } else {
    ...
    newFile <- paste("./2_LongestIsoforms/",i,"_longestIsoforms.fasta", sep = "")
    print(newFile)
